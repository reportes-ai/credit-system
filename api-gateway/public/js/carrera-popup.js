...
(function () {
  ...
  function mostrar(d) {
    ...
    const carril = (c, i) => {
      ...
      return `
      <div style="position:relative;height:44px;background:${laneColors[i % 2]};border-bottom:2px dashed rgba(255,255,255,.55)">
        <div style="position:absolute;left:10px;top:50%;transform:translateY(-50%);z-index:2;font-size:11px;font-weight:800;color:#fff;text-shadow:0 1px 3px rgba(0,0,0,.55);max-width:150px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${i + 1}. ${esc(c.nombre)}</div>
        <div class="af-cr-runner" data-pct="${pct}" style="position:absolute;left:0%;top:50%;transform:translateY(-50%);z-index:3;font-size:24px;animation:afCrCorre ${(0.5 + Math.random() * 0.3).toFixed(2)}s ease-in-out infinite;filter:drop-shadow(0 2px 3px rgba(0,0,0,.4))">${emoji}</div>
        <div class="af-cr-chip" style="position:absolute;left:0%;top:50%;transform:translate(30px,-50%);z-index:3;background:rgba(0,0,0,.55);color:#fff;border-radius:10px;padding:1px 8px;font-size:10.5px;font-weight:800;white-space:nowrap">${c.ops} <span style="font-weight:600;opacity:.8">/ ${meta}</span>${c.monto ? ` · <span style="color:#ffd740">${clp(c.monto)}</span>` : ''}</div>
      </div>`;
    };
    ...
    ov.innerHTML =
      '<div style="background:linear-gradient(160deg,#0a1d4d,#012d70 60%,#0141A2);border-radius:24px;padding:30px 34px 24px;color:#fff;max-width:760px;width:94vw;margin:16px;box-shadow:0 24px 80px rgba(0,0,0,.6);animation:afCrPop .5s ease">' +
      '<div style="text-align:center;font-size:1.25rem;font-weight:900;letter-spacing:1px">' + esc(d.titulo || '🏃 CARRERA DE COLOCACIONES') + '</div>' +
      '<div style="text-align:center;font-size:.85rem;opacity:.85;margin:5px 0 18px">' + esc(d.subtitulo || '') + '</div>' +
      // Pista: pasto + carriles tartán + partida y meta
      '<div style="background:#14532d;border-radius:14px;padding:14px 12px">' +
      ...
      '</div>' +
      '<div style="display:flex;justify-content:space-between;font-size:10px;color:#bbf7d0;margin-top:6px;padding:0 4px"><span>PARTIDA</span><span>META: ' + meta + ' créditos 🏁</span></div>' +
      '</div>' +
      '<div style="text-align:center;margin-top:20px"><button id="afCarreraBtn" style="background:#ffd740;color:#012d70;border:none;border-radius:26px;padding:11px 30px;font-weight:900;font-size:.92rem;cursor:pointer;box-shadow:0 4px 18px rgba(255,215,64,.45)">💪 ¡A correr!</button></div></div>';
