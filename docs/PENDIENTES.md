...
| 2.1b | **Disparador de `ODP_PAGADA` (y ANTICIPO/PRESTAMO/FINIQUITO_PAGADO)** | Detectado 27-08-2026 al agregar la "Cuenta de cargo" al pago de ODP: las reglas de egreso **existen y están activas** en Reglas de Centralización, pero **nadie las dispara** — pagar una ODP GENERAL (proveedores) hoy NO genera asiento (los pagos de saldo precio y comisión sí, vía Post Venta). Cablear `ODP_PAGADA` en `pagarOrden` (origen GENERAL) usando la cuenta de cargo ya registrada (`op_correlativos.id_cuenta_bancaria`); antes confirmar que el DEVENGO de esas facturas (2102010) esté entrando por el RCV/auxiliar para no descuadrar el pasivo. |
