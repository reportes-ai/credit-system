...
const ensureTable = async () => {
  ...
  //    para que no rompan el INSERT del esquema nuevo. MODIFY corre UNA vez (migrarAuto):
  //    era DDL repetido en cada boot (visto en TiDB SQL Statements).
  require('../../../../shared/migrate').migrarAuto('clientes_nullable_fix', async () => {
    const fixes = [
    ...
    for (const sql of fixes) {
      ...
      catch (e) { if (e.errno !== 1054) console.error('[clientes migration fix]', e.message); }
    }
  });
