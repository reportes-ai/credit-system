...
require('../../../../shared/migrate').enFila('comisiones', async () => {
  try {
    ...
    if (!hayModelos.n) {
      ...
      await pool.query('INSERT IGNORE INTO comisiones_modelos (nombre, descripcion, valores, es_sistema, creado_por) VALUES (?,?,?,1,?)',
        ['Modelo anterior (hasta jul-2026)', 'Piso $30M, 24 cuotas pagan 0,75% (≤ 24), cesantía 50% / reparaciones 30% / calidad 20% (3 créditos UNIDAD, todo o nada) con umbrales 65/50, sin RDH, bonos sobre el incentivo base total, semana corrida calculada por mes (art. 45 CT), sin descuentos por prepago.', JSON.stringify(anterior), 'Sistema']);
    }
