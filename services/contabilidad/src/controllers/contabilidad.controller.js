...
/* Otros impuestos del documento (específico, adicional, ILA…; Pato 15-09-2026): mismo
   tratamiento que el IVA no recuperable — AutoFácil no tiene giro que los recupere,
   así que son mayor costo (gasto) y parte de lo adeudado; nunca van a `iva` ni al F29. */
require('../../../../shared/migrate').enFila('ctb-compras-otros-imp', async () => {
  try { await pool.query('ALTER TABLE ctb_compras_aux ADD COLUMN otros_impuestos DECIMAL(14,0) NOT NULL DEFAULT 0'); }
  catch (e) { if (e.errno !== 1060) throw e; }
});
...
async function ingresarCompraAux(b, { origen = 'DIGITADO', req } = {}) {
  ...
  const otrosImp = Math.round(Math.abs(Number(b.otros_impuestos) || 0));
  const total = neto + exento + iva + ivaNoRec + otrosImp;
  ...
  if (b.generar_asiento) {
    ...
    const movimientos = [
      lado(cuenta_gasto, neto + exento + ivaNoRec + otrosImp, `${pre} ${num_doc} ${razon_social}`.slice(0, 200), !esNC),
  ...
  const [r] = await pool.query(
    `INSERT INTO ctb_compras_aux (mes, tipo_doc, num_doc, fecha_doc, fecha_vcto, estado, rut, razon_social, cuenta_cxp, cuenta_gasto, neto, exento, iva, iva_no_rec, otros_impuestos, total, origen, id_comprobante)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [mes, tipo_doc || '33', String(num_doc), fecha_doc, fecha_vcto || null, 'Vigente', rut, String(razon_social).slice(0, 200),
     cuenta_cxp || null, cuenta_gasto, neto, exento, iva, ivaNoRec, otrosImp, total, origen, comp?.id || null]);
...
async function calcularDJ1887(anio) {
  ...
  const [liq] = await pool.query(
    `SELECT mes, rut, nombre, imponible, total_ganado, haberes, afp_monto, salud_monto, seg_ces_trab, apv,
            impuesto_unico, mayor_retencion, prestamo_3pct, ias, total_imposicion, finiquito_otros,
            seg_ces_emp, sis_emp, aportes_emp_otros
  ...
  for (const r of liq) {
    ...
    if (!porRut.has(rut)) porRut.set(rut, { rut, nombre: r.nombre, renta: vacio12(), imp: vacio12(), mayor: vacio12(),
      nograv: vacio12(), p3: vacio12(), imponible: vacio12(), leyes: vacio12(), ias: vacio12(), fin: vacio12(), con: Array(12).fill(false) });
    ...
    /* Leyes sociales (C36) = cotizaciones que REBAJARON la renta neta (ganado − renta neta:
       AFP + cesantía + salud deducible + APV deducible) + las de cargo del empleador (cesantía
       y SIS del empleador, ley de accidentes y aportes de la reforma). Así lo arma AVSOFT:
       validado fila a fila contra su "Formato archivo plano F1887" AT2026 (281 de 322 filas con
       la fórmula anterior; las 41 restantes eran salud adicional no deducible y APV). */
    g.leyes[k] += Math.max(0, ganado - base) + (Number(r.seg_ces_emp) || 0) + (Number(r.sis_emp) || 0) + (Number(r.aportes_emp_otros) || 0);
    g.ias[k] += Number(r.ias) || 0;
    g.fin[k] += Number(r.finiquito_otros) || 0;
  }

  const act = (arr) => arr.map((v, k) => Math.round(v * (factor[k + 1] || 1)));
  const suma = (arr) => arr.reduce((a, x) => a + x, 0);
  const finInferido = [];
  // Sin renta, impuesto, no gravada ni retenciones en todo el año → no se informa (así lo hace AVSOFT).
  const enCero = [...porRut.values()].filter(g => ![g.renta, g.imp, g.mayor, g.nograv, g.p3].some(a => a.some(v => v)));
  const filas = [...porRut.values()]
    .filter(g => !enCero.includes(g))
    .sort((a, b) => Number(String(a.rut).split('-')[0]) - Number(String(b.rut).split('-')[0]))
    .map((g, i) => {
      const [cuerpo, dv] = String(g.rut).split('-');
      ...
      if (!u && ultimo < ultimoMes && !g.fin.some(x => x > 0) && !g.ias.some(x => x > 0)) finInferido.push(g.rut);
      const parcial = u && Number(u.jornada_especial_hrs) > 0 && Number(u.jornada_especial_hrs) <= 30;
      const siglas = g.con.map((c, k) => {
        if (!c) return '';
        const m = k + 1;
        if (m === baja || g.ias[k] > 0 || g.fin[k] > 0) return 'F';
        return parcial ? 'P' : 'C';
      });
  ...
  if (finInferido.length) avisos.push({ nivel: 'info', texto: `${finInferido.length} ex trabajador(es) sin ficha dejan de aparecer antes de fin de año sin pagos de finiquito en el libro; se informan con "C" (${finInferido.join(', ')}). Si alguno fue finiquitado, corresponde "F" en su último mes.` });
  if (enCero.length) avisos.push({ nivel: 'info', texto: `${enCero.length} RUT con liquidaciones en $0 todo el año no se informan: ${enCero.map(g => g.rut).join(', ')}.` });
  const faltan = [];
...
exports.listaComprasAux = async (req, res) => {
  try {
    ...
    const [meses] = await pool.query(
      `SELECT mes, COUNT(*) docs, SUM(neto) neto, SUM(exento) exento, SUM(iva) iva, SUM(iva_no_rec) iva_no_rec, SUM(otros_impuestos) otros_impuestos, SUM(total) total
         FROM ctb_compras_aux ${where} GROUP BY mes ORDER BY mes DESC`, p);
    const [[tot]] = await pool.query(`SELECT COUNT(*) docs, COALESCE(SUM(neto),0) neto, COALESCE(SUM(iva),0) iva, COALESCE(SUM(iva_no_rec),0) iva_no_rec, COALESCE(SUM(otros_impuestos),0) otros_impuestos, COALESCE(SUM(total),0) total FROM ctb_compras_aux ${where}`, p);
    ok(res, { documentos: docs, meses, total: tot, truncado: docs.length === 1000 });
  } catch (e) { fail(res, e.message); }
};
...
exports.rcvImportar = async (req, res) => {
  try {
    ...
    const [rcvRows] = await pool.query(
      `SELECT mes, tipo_dte, folio, rut_proveedor, monto_neto, monto_exento, iva_recuperable, iva_no_recuperable,
              otros_impuestos, imp_sin_derecho, monto_total FROM ctb_rcv_compras WHERE folio IN (?)`,
    ...
    for (const d of lista) {
      try {
        ...
        /* Montos: los del SII (el navegador solo aporta la cuenta). El documento debe
           cuadrar al peso con el total del RCV; si no, no entra y lo revisa una persona. */
        const [neto, exento, ivaRec, ivaNoRec, otrosSii, sinDer, totalSii] = ['monto_neto', 'monto_exento', 'iva_recuperable',
          'iva_no_recuperable', 'otros_impuestos', 'imp_sin_derecho', 'monto_total'].map(k => Number(sii[k] || 0));
        ...
        if (otros === null)
          throw new Error(`No cuadra con el total del SII ($${totalSii.toLocaleString('es-CL')}): faltan $${(resto - otrosSii).toLocaleString('es-CL')} sin impuesto informado — resincroniza el mes o digítalo a mano`);
