...
require('../../../shared/migrate').enFila('ctb-provisiones', async () => {
  ...
  await pool.query("INSERT IGNORE INTO ctb_config (clave, valor) VALUES ('prov_dealer_desde','2026-09')");
  await pool.query("INSERT IGNORE INTO ctb_cuentas (codigo, nombre, tipo, imputable) VALUES ('2106011','PROVISION COMISIONES DEALER','PASIVO',1)");
...
async function tick() {
  try { const r = await sincronizarDealer(); if (r.constituidas || r.liberadas) console.log('[provisiones-dealer]', JSON.stringify(r)); }
  catch (e) { console.error('[provisiones-dealer]', e.message); }
}
require('../../../shared/scheduler.js').programar('provisiones-dealer', tick, 6 * 60 * 60 * 1000, { arranqueMs: 3 * 60 * 1000 });

module.exports = { CONCEPTOS, constituirDealer, liberarDealer, liberarDealerPorNumOp, sincronizarDealer, cuadro };
