...
const DEFAULTS = [
  ...
  ['exc_piso3_pct',         25, 'Piso NIVEL 3: % mínimo de la alternativa más rentable (se exige ADEMÁS de las otras dos condiciones)'],
  ['exc_nivel3_estrellas',   3, 'Estrellas que cuesta el NIVEL 3 (piso 25% + rentabilidad ≥ comisión más alta + mínimo absoluto en $)'],
  ['exc_nivel3_factor_pct',  0, 'NIVEL 3: la rentabilidad AutoFácil debe ser AL MENOS la comisión MÁS ALTA (dealer o ejecutivo) recargada en este % (0 = al menos igualarla)'],
  ['exc_nivel3_rent_min', 200000, 'NIVEL 3: rentabilidad mínima ABSOLUTA de AutoFácil por operación ($); ninguna jugada de 3 estrellas puede quedar bajo esto'],
...
CLAVES.add('rev_unidad_activo'); CLAVES.add('rev_autofin_activo'); CLAVES.add('rev_unidad_tolerancia');
  ...
  try {
    const [rows] = await pool.query(
      `SELECT clave, valor, descripcion, fecha_actualizacion FROM parametros_credito
       WHERE clave LIKE 'exc\\_%' OR clave LIKE 'rev\\_unidad\\_%' OR clave LIKE 'rev\\_autofin\\_%' OR clave = 'pref_diff_pct' ORDER BY clave`);
    const obj = {};
