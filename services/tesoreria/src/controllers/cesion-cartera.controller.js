...
const fmtRut = r => { const n = RUT.normalizar(r); return n ? RUT.formatear(n) : String(r || ''); };   // 16.283.050-3
  ...
  ventas.forEach((v, i) => {
    ...
    html += `<div class="anexo-pag"><h3>ANEXO 2 — Individualización del crédito ${i + 1} de ${ventas.length}: operación N° ${v.num_op}</h3>
      ...
      <table class="ficha"><tbody>
        <tr><th>Deudor</th><td>${esc(v.cliente)}</td><th>RUT</th><td>${esc(fmtRut(v.rut_cliente))}</td><th>Operación / Pagaré</th><td>${v.num_op}</td></tr>
